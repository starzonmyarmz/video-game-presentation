...
export default function inViewport(elt, params, cb) {
  var opts = {
...
var contains = function() {
  ...
  return window.document.documentElement.compareDocumentPosition ?
    function (a, b) {
      return !!(a.compareDocumentPosition(b) & 16);
    } :
    window.document.documentElement.contains ?
      function (a, b) {
...
function createInViewport(container, debounceValue, failsafe) {
  ...
  var scrollContainer = container === window.document.body ? window : container;
  var debouncedCheck = debounce(watches.checkAll(watchInViewport), debounceValue);

  addEvent(scrollContainer, 'scroll', debouncedCheck);

  if (scrollContainer === window) {
    addEvent(window, 'resize', debouncedCheck);
  }
  ...
  function isVisible(elt, offset) {
    ...
    if (!contains(window.document.documentElement, elt) || !contains(window.document.documentElement, container)) {
      return false;
    }
